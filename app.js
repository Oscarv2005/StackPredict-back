...
app.post('/predict', async (req, res) => {
  try {
    // Explicitly using 127.0.0.1 to avoid local DNS/IPv6 routing resolution failures
    const response = await axios.post('http://127.0.0.1:5000/predict', req.body);
    res.json(response.data);
  } catch (err) {
    // Intercept upstream validation error states explicitly generated by Flask
    ...
    // Core structural offline fallbacks
    res.status(500).json({ 
      status: 'error',
